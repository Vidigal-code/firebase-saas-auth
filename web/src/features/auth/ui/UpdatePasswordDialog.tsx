...
export const UpdatePasswordDialog = ({ open, onClose }: Props) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      <DialogTitle>{t.auth.updatePasswordTitle}</DialogTitle>
      <DialogContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
          {error && <Alert severity="error">{error}</Alert>}
          {success && <Alert severity="success">{success}</Alert>}
          <TextField label={t.auth.oldPassword} type="password" fullWidth value={oldPassword} onChange={e => setOldPassword(e.target.value)} />
          <TextField label={t.auth.newPassword} type="password" fullWidth value={newPassword} onChange={e => setNewPassword(e.target.value)} />
        </Box>
      </DialogContent>
      <DialogActions sx={{ px: 3, pb: 2 }}>
        ...
        <Button onClick={onSubmit} variant="contained" disabled={isDisabled}>
          {loading ? t.auth.updating : t.auth.updateButton}
        </Button>
      </DialogActions>
    </Dialog>
  );
